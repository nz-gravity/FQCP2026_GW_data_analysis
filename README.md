...
| Metropolis–Hastings | 1 extension | marginals overlaid on the exact grid |
| NumPyro NUTS | 1 | sampled posterior overlaid on the exact grid |
| teaching NUTS mechanics | 1 extension | trajectories and effective sample size compared with Metropolis |
| variational inference | 1 extension | mean-field and full-rank fits compared with the exact posterior and evidence bound |
| Fisher / Laplace extensions | 1 and 3A | exact for the linear model; `sigma(ln A) = 1/SNR` for LISA |
| nested sampling | 1 extension | `log Z` agrees with the grid evidence to ~0.1 |
...
The Bayesian, LVK, and LISA notebooks contain pinned, conditional install
cells. Python 3.12 is used by the locked environment. No GPU is required.
...
`uv.lock` is the authoritative, reproducible local environment; update it
deliberately with `uv lock` after changing `pyproject.toml`. The Jupytext files
in `notebook_sources/` are the reviewable sources. The matching committed
`.ipynb` files in `notebooks/` are generated, output-free artifacts retained so
that every chapter opens directly in Google Colab. Edit the Jupytext source,
then run `bash scripts/build_notebooks.sh`; do not hand-edit notebook JSON.
`build_site.py` builds those notebooks into `_build/html`; the book
configuration and navigation are in `_config.yml` and `_toc.yml`.
